...
const SessionData = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const json = await sessionService.getAll();
            setDocs(json);
        } catch (err) {
    ...
    const handleSave = async () => {
        try {
            ...
            const body = { [keyInput]: parsedValue };

            if (editingItem && editingItem.docId) {
                await sessionService.update(editingItem.docId, body);
            } else if (targetDocId) {
                await sessionService.update(targetDocId, body);
            } else if (docs.length > 0) {
                const latest = docs[0];
                await sessionService.update(latest._id, body);
            } else {
                await sessionService.create(body);
            }

            // Service throws on error, so we catch it below
        ...
        showConfirm(
            ...
            async () => {
                try {
                    ...
                    await sessionService.update(item.docId, { [item.key]: null });
